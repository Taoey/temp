...
//单条删除
    function godel(id){
       if(confirm("确定删除信息吗？")){
           ...
           window.location="leaseShuiguo.do?method=delShuiguo&fromto=admin&id="+id;
       }
    }
//批量删除
  function dels(id){
	...
	{
	  if(boxs[i].checked){
		 leaseShuiguo.delShuiguo(boxs[i].value);         
      }else
	  {
	       check++;
	  }
    ...
    function showShuiguoInfo(rid){
      ...
      document.getElementById("sdiv").style.filter="alpha(opacity=10)";//设置元素的不透明级别
      document.getElementById("sdiv").style.display = "block";//此元素将显示为块级元素，此元素前后会带有换行符。
      showImg(rid);
      showShuiguo(rid);
    ...
    function showShuiguo(rid){
       leaseShuiguo.getShuiguo(rid,function(shuiguo){
          document.getElementById("title").innerHTML=shuiguo.title;//获取标题
          document.getElementById("area").innerHTML=shuiguo.areaType.area;//获取区域
          document.getElementById("address").innerHTML=shuiguo.address;//获取地址
          document.getElementById("colore").innerHTML=shuiguo.colore;//获取颜色
          document.getElementById("sort").innerHTML=shuiguo.sort;//获取类别
          document.getElementById("kind").innerHTML=shuiguo.kind;//获取种类
          document.getElementById("type").innerHTML=shuiguo.shuiguoType.type;//获取品种
          document.getElementById("acreage").innerHTML=shuiguo.acreage;//获取数量
          document.getElementById("price").innerHTML=shuiguo.pirce;//获取价格
          if(shuiguo.esthment==null){
            document.getElementById("esthment").innerHTML="";
          }else{
            document.getElementById("esthment").innerHTML=shuiguo.esthment;//获取运输方式
          }
         
          document.getElementById("remark").innerHTML=shuiguo.remark;//获取说明
          if(shuiguo.users.info!=null&&shuiguo.users.info!=""){
            document.getElementById("uname").innerHTML=shuiguo.users.truename;//获取名字
          }else{
            document.getElementById("uname").innerHTML=shuiguo.users.uname;
          }
          
          document.getElementById("phone").innerHTML=shuiguo.users.phone;//获取电话
          document.getElementById("email").innerHTML=shuiguo.users.email;//获取email
       });
    }
     
function isact(id){
	 if(confirm("通过审核？")){
		window.location="leaseShuiguo.do?method=toExamine&id="+id+"&state=3";
	}
 }
	
function act(id){
	 if(confirm("未通过审核？")){
		window.location="leaseShuiguo.do?method=toExamine&id="+id+"&state=2";
	}
 }			 
  </script>
		...
		<table width="98%" border="0" cellpadding="2" cellspacing="1"
			...
			<logic:notEmpty name="rlList">
				<logic:iterate id="shuiguo" name="rlList" indexId="i">
					<tr align='center' bgcolor="#FFFFFF"
						...
						<td>
							<input name="boxs" type="checkbox" id="boxs" value="${shuiguo.id }"
								class="np">
						</td>
						<td align="left">
							${shuiguo.title }
						...
						<td>
								...
								<c:if test="${shuiguo.business.bcontact==null }">
              ...
              ${shuiguo.business.bcontact}
						...
						<td>
							<logic:equal name="shuiguo" property="state" value="1">
								<a href="javascript:isact(${shuiguo.id });"><FONT color="green">通过</FONT></a>&nbsp;&nbsp;
								<a href="javascript:act(${shuiguo.id });"><FONT color="red">未通过</FONT></a>
							</logic:equal>
							...
							<logic:equal name="shuiguo" property="state" value="4">
								<FONT color="green"><br></FONT>
							</logic:equal>

						</td>
						<td>
                            <a href="actTable.do?method=act&id=${shuiguo.id }">查看举报信息</a>
						</td>
						<td>
							<a href="javascript:godel(${shuiguo.id });">删除</a> |
							<a href="javascript:showShuiguoInfo(${shuiguo.id });">详细</a>
						</td>
					</tr>
				</logic:iterate>
			</logic:notEmpty>
			...
			<tr bgcolor="#EEF4EA">
				<td height="36" colspan="10" align="right">
					...
					<a href="leaseshuiguo.do?method=showShuiguoAdmin&showPage=1"
						class="coolbg">首页</a>
					<logic:notEqual name="showPage" value="1">
						<a
							href="leaseShuiguo.do?method=showShuiguoAdmin&showPage=${showPage-1 }"
							class="coolbg">上一页</a>
					</logic:notEqual>
					...
					<a href="leaseShuiguo.do?method=showShuiguoAdmin&showPage=${allPage}"
						class="coolbg">尾页</a>
				</td>
			</tr>
		</table>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0"
						...
						<tr>
							<td align="left" width="50%">
								<font size="5" color="#FF0099">&nbsp;实习信息</font>
							</td>
							<td align="right" style="font-size: 12px; font-weight: bold">
								<span onclick="hiddShuiguoInfo();" style="cursor: pointer;"><img
					...
					<table width="96%" border="0" cellspacing="4" cellpadding="1">
						<tr>
							...
							<td align="left" style="font-size: 12px; font-weight: bold">
								...
								<br>
								细则：
								<span id="colore"></span>结账方式;&nbsp;
								<span id="sort"></span>天/周（工作）;&nbsp;
								<span id="kind"></span>工作经验；&nbsp; 学历：
								<span id="type"></span>
								<br>
								<br>
								人数：
								<span id="acreage"></span>人；&nbsp; 工资水平：
								<span id="price"></span>元/天
								<br>
								<br>
								公司待遇：
