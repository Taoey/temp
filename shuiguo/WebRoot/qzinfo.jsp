...
<script type="text/javascript">
   function addchoosshuiguo(id,uid){
      cshuiguo.add(id,uid,function(msg){
         alert(msg);
      });
   }
</script>
...
<DIV class=logo><A href="index.jsp"><IMG alt=eeju src="image/logo.jpg" width="180" height="50"></A></DIV>
</DIV>
...
<c:if test="${beg.users.uid!=null }">
     <c:if test="${beg.users.image==''}">
        <IMG alt="" src="images/QiTSPRJFM.jpg" width=120 height=120>
     </c:if>
     <c:if test="${beg.users.image!='' }">
        <IMG alt="" src="uploadIMG/${beg.users.image}" width=120 height=120>
     </c:if>
...
<UL class="broker_links">
  <LI>求职者：<STRONG>     
            <c:if test="${beg.users.truename=='' }">
              ${beg.users.uname}
            </c:if>
            <c:if test="${shuiguo.users.truename!='' }">
              ${beg.users.truename }
            </c:if>
...
<UL class=shuiguo_info2>
  <li>工资水平： ${beg.minprice }-${beg.maxprice } 元/天</li>
  <li class=info2_long>发布时间：${beg.time }<I></I></li>
  <LI class=info2_short>所在区域：${beg.areaType.area }</LI>
  <LI class=info2_short>学历水平： ${beg.shuiguoType.type }</LI>
  <LI class=info2_short>细则：${beg.colore }结账方式/每周可工作${beg.sort }天/${beg.kind }工作经验</LI>
  <LI class=info2_short>招聘人数：${beg.minarceage }~${beg.maxarceage }</LI>
  <LI class=info2_long>公司待遇：${beg.esthment} </LI>
  <LI class=info2_long>地址：${beg.address }
  </LI></UL>
